 ...
 *
 * All work here is streaming: encrypt/decrypt copy [InputStream] to [OutputStream] without holding
 * the whole payload in memory, so large files are fine. The byte[] helpers exist only for small
 * inputs (pasted text, key material).
 *
...
object AgeCrypto {
    ...
    fun encryptBytes(recipients: List<Recipient>, plaintext: ByteArray, armor: Boolean): ByteArray {
    ...
    fun decryptBytes(identities: List<Identity>, ciphertext: ByteArray): ByteArray {
        val out = ByteArrayOutputStream()
